...
async function initFirebase() {
  ...
  try {
    ...
    _monitorConnection();
    _checkURLRoom();
    // Delay _patchButtons so episode-feature.js (loaded before us) has
    // already wrapped window.playById on its own DOMContentLoaded handler.
    // We patch LAST so our wrapper sits outermost in the chain.
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => setTimeout(_patchButtons, 0));
    } else {
      setTimeout(_patchButtons, 0);
    }
